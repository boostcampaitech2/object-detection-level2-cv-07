...
def get_net(checkpoint_path=None):
    ...
    config = get_efficientdet_config('efficientdet_d4')
...
def train_fn(num_epochs, train_data_loader, optimizer, model, device, clip=35):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(), f'./work_dir/EfficientDet_d4/epoch_{epoch+1}.pth')
        wandb.log({"Cls_Loss": cls_loss,"Box_Loss": box_loss,"Loss": loss})
...
def main():
    train_annotation = '../dataset/train.json'
    data_dir = '../dataset'
    ...
    wandb.config.update({'name':model.config.name,'pretrained_model':model.config.backbone_name,
                        ...
                        'optimizer':'AdamW',
                        'lr':optimizer.state_dict()['param_groups'][0]['lr'],'epoch': num_epochs})
