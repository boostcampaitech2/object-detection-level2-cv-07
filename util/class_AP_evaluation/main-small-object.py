...
small_size = (0, 10000)      # small object area 기준
...
def main(ground_truth_file, predict_file):
    ...
    for truth in ground_truth['annotations']:
        if truth['bbox'][2]*truth['bbox'][3] <= small_size[1]:
    ...
    for pred in predict['annotations']:
        if small_size[0] <= (pred['bbox'][2] - pred['bbox'][0])*(pred['bbox'][3] - pred['bbox'][1]) <= small_size[1]:
